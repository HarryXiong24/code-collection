...
func showBool() {
	...
	fmt.Println("true || false", a || b)
	// GO 语言中，真-true，假-false,不与数值相等。 (其他语言 true 1 false 0)
}
...
func main() {
	// 一般是用 int 表示整型的宽度，在32位系统下，就是32位，在64位系统下，就是64位。
	Integer()
	fmt.Println("-----------------")
	unsignedInteger()
	fmt.Println("-----------------")
	...
	study = "Go"     // 赋值
	fmt.Println(study)
	study2 := "golang" // 短声明方式，自动推断类型并初始化
	fmt.Println(study2)

	var s1 string
	s1 = `
			var study string      // 定义 study 字符串型变量
			study = "Go" // 赋值
			fmt.Println(study)
		`
	fmt.Println(s1)
