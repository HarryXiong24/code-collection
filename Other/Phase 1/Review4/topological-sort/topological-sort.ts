...
export class Graph {
  ...
  topologicalSort() {
    const stack: number[] = [];
    const visited = new Array(this.vertices).fill(Color.White);

    const recursion = (vertex: number): boolean => {
      visited[vertex] = Color.Gray;

      if (this.adjacentList.has(vertex)) {
        for (const item of this.adjacentList.get(vertex)!) {
          if (visited[item] === Color.Gray) {
            return true;
          }
          if (visited[item] === Color.White && recursion(item)) {
            return true;
          }
        }
      }
