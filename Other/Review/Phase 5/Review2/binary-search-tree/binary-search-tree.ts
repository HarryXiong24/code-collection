    ...
    const recursion = (node: TreeNode | null, parent: TreeNode | null): boolean => {
      ...
      if (value < node.val) {
        ...
        if (!node.left || !node.right) {
          ...
          let current_min_node = node.right;
          let current_parent = node;

          while (current_min_node.left) {
            current_parent = current_min_node;
            current_min_node = current_min_node.left;
          }

          node.val = current_min_node.val;

          if (current_parent.left === current_min_node) {
            current_parent.left = current_min_node.right;
          } else {
            current_parent.right = current_min_node.right;
          }
        }
