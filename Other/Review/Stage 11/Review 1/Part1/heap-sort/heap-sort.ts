export function heapify(nums: number[], length: number, current_index: number) {
...
// The idea is simple:
// transformed value = −(original value)
// Store -x instead of x in a max-heap.
// Max-heap will now treat the largest -x as the root.
// Observation:
// Largest -x → corresponds to smallest x. ✅
