...
export function solveNQueens(n: number): string[][] {
  ...
  const isValid = (row: number, col: number, chessboard: string[][]): boolean => {
    for (let i = 0; i < row; i++) {
    ...
    for (let i = row - 1, j = col + 1; i >= 0 && j < chessboard[i].length; i = i - 1, j = j + 1) {
      if (chessboard[i][j] === 'Q') {
        return false;
      }
    }
