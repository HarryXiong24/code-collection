...
export function dijkstra(graph: Graph, start: string, end: string): [string[], number] {
  const dijkstra_table: Record<string, number> = Object.keys(graph).reduce((obj, attr) => {
    obj[attr] = Infinity;
    return obj;
  }, {} as Record<string, number>);
  const record_path: Record<string, string | null> = Object.keys(graph).reduce((obj, attr) => {
    obj[attr] = null;
    return obj;
