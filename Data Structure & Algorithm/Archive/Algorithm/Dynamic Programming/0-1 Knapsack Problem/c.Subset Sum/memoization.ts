...
export function subsetSum(arr: number[], total: number) {
  const dp: (boolean | null)[][] = [...Array(arr.length)].map(() => Array(total + 1).fill(null));
  ...
  const recursive = (arr: number[], total: number, n: number): boolean => {
    ...
    if (dp[n][total] !== null) {
      return dp[n][total] as boolean;
    }

    if (arr[n] > total) {
      // Recursive calls
      dp[n][total] = recursive(arr, total, n - 1);
      return dp[n][total] as boolean;
    }

    // we either exclude the element or include the element
    dp[n][total] = recursive(arr, total, n - 1) || recursive(arr, total - arr[n], n - 1);
    return dp[n][total] as boolean;
  };
