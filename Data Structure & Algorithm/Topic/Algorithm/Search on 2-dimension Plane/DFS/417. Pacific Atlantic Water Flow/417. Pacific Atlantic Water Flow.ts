...
export function pacificAtlantic(heights: number[][]): number[][] {
  ...
  const recursive = (i: number, j: number, set: Set<string>, prev: number) => {
    ...
    for (const item of Ocean) {
      recursive(i + item[0], j + item[1], set, heights[i][j]);
    }
  };

  for (let col = 0; col < heights[0].length; col++) {
    // left side pacific
    recursive(0, col, pacificSeen, -Infinity);
    // right side atlantic
    recursive(heights.length - 1, col, atlanticSeen, -Infinity);
  }
  for (let row = 0; row < heights.length; row++) {
    // top side pacific
    recursive(row, 0, pacificSeen, -Infinity);
    // bottom side atlantic
    recursive(row, heights[0].length - 1, atlanticSeen, -Infinity);
  }
