...
/** 打印一步操作和它的返回值，对齐成 `label  →  result`。 */
export function cmd(label: string, result: unknown): void {
  console.log(`  ${C.green}${label.padEnd(44)}${C.reset}${C.dim}→${C.reset} ${format(result)}`);
}
...
function format(v: unknown): string {
  if (v === null || v === undefined) return `${C.dim}(none)${C.reset}`;
